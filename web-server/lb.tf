...
##########################################################################################
resource "oci_load_balancer_load_balancer" "lb-web" {
...
resource "oci_load_balancer_backend_set" "lb-backendset-web" {
  name             = "lb-backendset-web"
  load_balancer_id = oci_load_balancer_load_balancer.lb-web.id
  policy           = "ROUND_ROBIN"
...
resource "oci_load_balancer_backend" "lb-backend-web" {
  ...
  ip_address       = oci_core_instance.WebServer[count.index % length(oci_core_instance.WebServer)].private_ip
  port             = 80
  backup           = false
  drain            = false
  offline          = false
...
##########################################################################################
output "lb_ip" {
  value = [oci_load_balancer_load_balancer.lb-web.ip_address_details[0].ip_address]
}
output "lb_is_public" {
  value = [oci_load_balancer_load_balancer.lb-web.ip_address_details[0].is_public]
}
